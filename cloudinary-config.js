...
/**
 * Generate a mockup using Cloudinary's URL API
...
async function generateMockupWithCloudinary(logoUrl, options = {}) {
  try {
    console.log("Generating mockup with Cloudinary using URL API...");
    ...
    console.log("Creating new mockup with ID:", mockupId);
    
    // Instead of trying to use the API, we'll use a much simpler approach:
    // We'll create a URL that directly references the background and overlays the logo
    ...
    // Create a new image by directly manipulating the URL
    // This is a much simpler approach that avoids the API
    
    // Start with the base URL for the background
    let mockupUrl = `https://res.cloudinary.com/${process.env.CLOUDINARY_CLOUD_NAME}/image/upload/c_fill,h_1080,w_1920/backgrounds/default-bg`;
    
    // Add the logo overlays for slippers
    for (const pos of slipperPositions) {
      const x = Math.floor(pos.x - slipperWidth / 2);
      const y = Math.floor(pos.y - slipperHeight / 2);
      mockupUrl = `${mockupUrl}/l_${logoPublicId},c_scale,g_north_west,h_${slipperHeight},w_${slipperWidth},x_${x},y_${y}`;
    }
    
    // Add the logo overlays for labels
    for (const pos of labelPositions) {
      const x = Math.floor(pos.x - labelWidth / 2);
      const y = Math.floor(pos.y - labelHeight / 2);
      mockupUrl = `${mockupUrl}/l_${logoPublicId},c_scale,g_north_west,h_${labelHeight},w_${labelWidth},x_${x},y_${y}`;
    }
    
    // Add a version to the URL
    mockupUrl = `${mockupUrl}.png`;
