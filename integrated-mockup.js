...
const MANYCHAT_API_KEY = process.env.MANYCHAT_API_KEY || '7044882:cc66bd757dd062d297296fb85610330b';
const MANYCHAT_FIELD_NAME = 'mockup_url';

// WhatsApp redirect URL
const WHATSAPP_REDIRECT_URL = 'https://api.whatsapp.com/send?phone=5551994305831&text=Ola.%20tenho%20interesse%20em%20Chinelos%20Slide%20personalizados%20HUD%20LAB%20Greco';
...
app.post('/api/mockup', upload.single('logo'), async (req, res) => {
  try {
    ...
    await activeCampaign.processLeadWithMockup({ email, name, phone }, publicUrl);
...
app.post('/api/webhook/active-campaign', async (req, res) => {
  try {
    ...
    if (!contact || !contact.email || !mockup_url) {
      return res.status(400).json({ error: 'Contact email and mockup_url are required' });
    }
