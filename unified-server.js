...
/**
 * API endpoint for basic user data submission
 * Handles only the user data without logo, for faster form submission
 */
app.post("/api/user-data", express.json(), async (req, res) => {
  try {
    console.log("Received basic user data submission");
    ...
    console.log(`Processing basic data for user: ${email}`);

    // Process lead in ActiveCampaign immediately
    ...
    res.json({
      success: true,
      message: "User data received and being processed",
...
/**
 * API endpoint for logo upload and mockup generation
...
app.post("/api/mockup", upload.single("logo"), async (req, res) => {
  try {
    ...
    // Send an early response to the client before processing the logo
    // This allows the client to continue without waiting for logo processing
    const earlyResponse = {
      success: true,
      message: "Logo received and being processed",
      redirect_url: REDIRECT_URL,
    };

    console.log("Sending early response to client before logo processing");
    res.json(earlyResponse);
