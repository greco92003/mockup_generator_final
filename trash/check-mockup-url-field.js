...
async function checkContactCustomField(email, fieldName) {
  try {
    ...
    const customField = customFields.find(field => field.title.toLowerCase() === fieldName.toLowerCase());
    ...
    const fieldValue = fieldValues.find(value => value.field === customField.id);
    
    if (!fieldValue) {
      console.log(`Custom field ${fieldName} not set for contact ${contact.id}`);
      return null;
    }
    
    console.log(`Custom field ${fieldName} value for contact ${contact.id}: ${fieldValue.value}`);
    return fieldValue;
  } catch (error) {
