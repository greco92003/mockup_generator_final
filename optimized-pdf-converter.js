...
class PdfConverter {
  ...
  async convertPdfToPng(buffer, filename) {
    ...
    // Generate cache key
    const cacheKey = this.cacheManager.generateKey(buffer, filename.replace(/\.pdf$/i, '.png'));
    ...
    try {
      ...
      await this.cloudConvert.tasks.upload(uploadTask, buffer, filename, buffer.length);
