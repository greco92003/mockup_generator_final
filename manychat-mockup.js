...
// Upload mockup to Supabase Storage and save metadata
async function uploadMockupToSupabase(mockupPath, email, name, phone) {
  try {
    ...
    if (metaError) {
      console.error('Error saving to Supabase Database:', metaError);
      throw metaError;
    }
    ...
    console.error('Error in uploadMockupToSupabase:', error);
    throw error;
  }
}
...
app.post('/api/mockup', upload.single('logo'), async (req, res) => {
  try {
    ...
    const publicUrl = await uploadMockupToSupabase(mockupPath, email, name, phone);
...
app.get('/api/manychat/mockup', async (req, res) => {
  try {
    ...
    // Query the database for the most recent mockup for this email
    const { data, error } = await supabase
      .from('mockup_leads')
      .select('mockup_url, created_at, name, phone')
      .eq('email', email)
    ...
    res.json({
      ...
      content: {
        messages: [
          ...
          {
            type: 'text',
            text: `Aqui está o seu mockup, ${data[0].name}! Esperamos que goste do resultado.`
          }
        ]
      }
    });
...
// Webhook for ManyChat to trigger sending mockup to WhatsApp
app.post('/api/manychat/send-mockup', async (req, res) => {
  try {
    const { email, subscriber_id, phone } = req.body;
    ...
    if (!subscriber_id) {
      return res.status(400).json({ error: 'ManyChat subscriber_id is required' });
    }
    
    // Query the database for the most recent mockup for this email
    const { data, error } = await supabase
      .from('mockup_leads')
      .select('mockup_url, created_at, name')
      .eq('email', email)
      .order('created_at', { ascending: false })
      .limit(1);
    ...
    // Here you would integrate with ManyChat API to send the mockup
    // This is a placeholder for the actual ManyChat API integration
    console.log(`Would send mockup ${data[0].mockup_url} to subscriber ${subscriber_id} via ManyChat`);
    
    // Return success response
    res.json({
      success: true,
      message: 'Mockup sent to ManyChat for delivery',
      mockup_url: data[0].mockup_url
    });
  } catch (error) {
    console.error('Error in ManyChat send webhook:', error);
    res.status(500).json({ error: 'Failed to send mockup to ManyChat' });
  }
});
